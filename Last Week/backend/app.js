...
var connectionString = 'mongodb://testuser1:testuser1!@cluster0-shard-00-00-ufyzn.mongodb.net:27017,cluster0-shard-00-01-ufyzn.mongodb.net:27017,cluster0-shard-00-02-ufyzn.mongodb.net:27017/Mobile?ssl=true&replicaSet=Cluster0-shard-0&authSource=admin';
...
app.get('/api/subjects/:id', function(request, response){
	...
	Subject.findById(request.params.id, function(error, subject){
...
app.get('/api/chapters/:id', function(request, response){
	...
	Chapter.findById(request.params.id, function(error, subject){
...
app.post('/api/user', function(request, response) {
	...
	new_user._id = request.body.email;
	...
	new_user.save( function(error) {
		...
		if(error) {
			...
			if(error.code === 11000) {
				...
				response.json({ message: error.message, data: [] });
